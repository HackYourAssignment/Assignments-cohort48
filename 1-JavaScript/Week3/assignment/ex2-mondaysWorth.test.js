...
function computeEarnings(tasksList, hourlyRate) {
  const taskRate = tasksList.map((task) => (task.duration*hourlyRate)/60 ).reduce((sum, rate) => sum + rate)
  return `€${taskRate.toFixed(2)}`
