...
function rollDice() {
  ...
  const promises = dice.map(d => rollDie(d));
...
because the `setTimeout` function is still working. Even if a promise fails, the
`setTimeout` does not stop. So, the other dice will finish rolling and show their
